		...
		<b-container fluid>
			<b-navbar id="nav" toggleable="sm" class="py-2" >						
				<b-collapse id="nav-collapse" is-nav>
					<b-navbar-nav small class="align-items-center">
						<b-nav-item class="brand"><span v-if="loading || loadingCharts || loadingTracks">Loading...</span><span v-else>JPP-UI</span></b-nav-item>
			...
			<b-tabs v-show="nav == 'Races'">
				<b-tab v-for="(race, racekey) in races" :key="racekey">
					...
					<horse-view :race="race" :hideML="hideML" :charts="charts" @selectionUpdate="tmGenerate" @togglePick="togglePick" @goToChart="goToChart"></horse-view>
				</b-tab>
			</b-tabs>
			...
			<div v-show="nav == 'Horses'" class="mx-5">
				<horses-to-watch-view></horses-to-watch-view>
			</div>
...
export default {
	...
	computed: {
		...
		chartDates() {
			...
			return _.pluck(_.where(this.charts, {track: this.track}),'date');
		},
		raceDates() {
			var str_pad_left = this.str_pad_left;
			if (!this.track) return [];
			if (_.pluck(this.tracks, "code").indexOf(this.track) == -1) return [];
			return _.map(		
					_.find(this.tracks, {code: this.track}).raceDates			
				, function (d) {
					return d[0] + "-" 
	...
	methods: {
		...
		disableDates(ymd, date) {
			const day = "" 
				+ this.str_pad_left((date.getMonth()+1),0,2) 
				+ this.str_pad_left(date.getDate(),0,2) 
				+ date.getFullYear();
			return this.chartDates.indexOf(day) ==  -1;
		},
		async goToChart(pp) {
			this.nav = "Charts";
			this.track = pp.trackCode;
			this.chartDate = null;
			this.chartRaceFromPP = pp.raceNumber - 1;
			try {
				this.loading = true;
				const response = await axios({
					url: 'getChart/' + this.track + "/" + pp.raceDate[0] + "/" + pp.raceDate[1]+ "/" + pp.raceDate[2],
					method: 'GET',
					baseURL: 'http://localhost:8080/jpp/rest/remote/'
				});
				this.chart = response.data;
		...
		raceDatesClass(ymd, date) {	
			...
			const track = _.where(this.charts, {track: this.track});
			const chart = _.findWhere(track, {date: day});
			
			if (chart && chart.hasNotes) return "table-success";
			if ((this.raceDates.indexOf(ymd) > -1) && (this.chartDates.indexOf(day) > -1)) return "table-danger";
			if (this.raceDates.indexOf(ymd) > -1) return "table-primary";	
			if (this.chartDates.indexOf(day) > -1) return "table-info";
		},
			...
			if (this.raceDates.indexOf(ymd) ==  -1) {
				...
				for (var i = 0; i < this.tracks.length; i++) {
					if (this.tracks[i].code == this.track) {
						found = true;
						this.tracks[i].raceDates.push(day);
					}
				}
				if (!found) {
					this.tracks.push({
				...
				for (i = 0; i < this.tracks.length; i++) {
					if (this.tracks[i].code == this.track) {
						this.tracks[i].raceDates = _.reject(this.tracks[i].raceDates, function(d){
							return (d[0] == day[0] && d[1] == day[1] && d[2] == day[2]);
						})
					}
				}
			}
