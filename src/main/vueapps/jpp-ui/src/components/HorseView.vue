<template>
    ...
    >
        ...
        <template #cell()="row">
            <span v-b-tooltip.hover :title="(row.item.scratchedFlag) ? '' : row.field.label + ((row.field.rank) ? ' (' + rankOf(row.unformatted, row.field.key, row.field.reverse) + ' of ' + race.unscratchedHorses.length + ')' : '')">{{row.value}}</span>
        </template>         
        ...
        <template #cell(mlodds)="row">       
                <span class="text-danger" v-if="hideML" v-b-tooltip.hover :title="(row.item.scratchedFlag) ? '' : row.field.label + ' (' + rankOf(row.unformatted, row.field.key, row.field.reverse) + ' of ' + race.unscratchedHorses.length + ')'"><b-icon-eye-slash-fill></b-icon-eye-slash-fill></span>
        ...
        <template #cell(classRating)="row">
            <span v-b-tooltip.hover.html :title="(row.item.scratchedFlag) ? '' : row.field.label 
                + ' (' + rankOf(row.unformatted, row.field.key, row.field.reverse) + ' of ' + race.unscratchedHorses.length + ')'
                + (row.item.brisCurrentClass > 0 ? '<br>BRIS Cur: ' + format2Places(row.item.brisCurrentClass) + ' (' + rankOf(row.item.brisCurrentClass, 'brisCurrentClass', row.field.reverse) + ' of ' + race.unscratchedHorses.length + ')': '') 
                + (row.item.brisAvgLast3Class > 0 ? '<br>BRIS Avg L3: ' + format2Places(row.item.brisAvgLast3Class) + ' (' + rankOf(row.item.brisAvgLast3Class, 'brisAvgLast3Class', row.field.reverse) + ' of ' + race.unscratchedHorses.length + ')': '')">{{row.value}}</span>
        </template>    
        <template #cell(aratingForm)="row">
            <span v-b-tooltip.hover :title="'Basic Fitness: ' + row.item.basicFitness">{{row.value}}</span>
        </template>
        ...
        <template #row-details="row">
            <b-container v-if="row.item.scratchedFlag" fluid class="pp border scratchedDetail text-center m-2 p-2">
                <span class="h4 text-white">{{row.item.name}} is scratched</span>
            </b-container>
                        ...
                        <b-row>
                            ...
                            <b-col>
                                <span class="h5"><strong>
                                    <b-icon-star-fill 
                                        variant="success" 
                                        v-if="row.item.horseToWatch && row.item.horseToWatch.flag == 'Star'"
                                        v-b-popover.hover.html.right="{content: formatHorseToWatch(row.item.horseToWatch), title: 'Horse To Watch'}"                                       
                                    ></b-icon-star-fill>
                                    <b-icon-binoculars
                                        variant="success" 
                                        v-if="row.item.horseToWatch && row.item.horseToWatch.flag != 'Star'"
                                        v-b-popover.hover.html.right="{content: formatHorseToWatch(row.item.horseToWatch), title: 'Horse Notes'}"                                       
                                    ></b-icon-binoculars>
...
<script>
import { BIconTypeUnderline, BIconTypeStrikethrough, BIconInfo, BIconEyeSlashFill, BIconCloudUploadFill, BIconStarFill, BIconBinoculars } from 'bootstrap-vue'
...
export default {
    name: 'HorseView',
    components: {
		PastPerformanceView, WorkoutView, HorseExtraView, BIconTypeUnderline, BIconTypeStrikethrough, BIconInfo, BIconEyeSlashFill, BIconCloudUploadFill, BIconStarFill, BIconBinoculars
    },
    ...
    data () {
		return {
			...
			horseFields: [
				...
				{key: "mlodds", "label": "ML", title: "Morning Line Odds", sortable:true, tdClass: "text-center", rank: true, reverse: true},
    ...
    methods:  {
            ...
            try {
                ...
                formData.append("data", JSON.stringify({
                    ...
                    raceNotes: [{
                        ...
                        raceNumber: this.race.raceNumber,
                        position: horse.finishPosition,
                        comment: "Added from PP"
                    }]
                }));
                const response = await axios({
                    url: 'addHorseToWatch',
		...
		async removeHorseToWatch(horseToWatch) {
            try {
				this.loading = true;
                var formData = new FormData();
                formData.append("data", JSON.stringify(horseToWatch));
                await axios({
                    url: 'removeHorseToWatch',
                    method: 'POST',
                    baseURL: 'http://localhost:8080/jpp/rest/remote/',
                    headers: {
                        'Content-Type': 'multipart/form-data'
        ...
        highlightName (value, key, item) {
            if (item.horseToWatch && item.horseToWatch.flag == 'Star') return "star";
        },     
		...
		highlightPaceAdvantage(value, key, item) {
			if (_.contains(_.pluck(this.race.advantagedHorses,"name"), item.name)) return "greenHighlight";
		},
        ...
        rankOf(value, property, reverse) {
            var sa = _.pluck(this.race.unscratchedHorses, property);
        ...
        formatHorseToWatch(data) {
            var text = (data.comment) ? data.comment + "<br>" : "";
            for (var i = 0; i < data.raceNotes.length; i++) {
                text += "<li>"
                    + this.formatDate(data.raceNotes[i].raceDate) 
                    + "<sup>" 
                    + data.raceNotes[i].raceNumber
                    + "</sup> "
                    + data.raceNotes[i].track
                    + ": ("
                    + data.raceNotes[i].position
                    + ((data.raceNotes[i].beatenLengths > 0) ? "<sup>" + data.raceNotes[i].beatenLengths + "</sup>" : "")
                    + ") "
                    + data.raceNotes[i].flag
                    + ((data.raceNotes[i].comment) ? " - "+ data.raceNotes[i].comment : "")
                    + "</li>";
